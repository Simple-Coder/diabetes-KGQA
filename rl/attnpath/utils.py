...
def teacher(e1, e2, num_paths, env, path=None, output_mode=0, relation=None, knowledge_base=None):
    ...
    for entities, relations in zip(res_entity_lists, res_path_lists):
        ...
        duplicate_ents.sort(key=lambda x: x[1], reverse=True)  # Sort in reverse according to the occurence times
        for item in duplicate_ents:
            ent = item[0]  # entity
            ent_idx = [i for i, x in enumerate(rel_ents) if x == ent]  # i: index, x: value of index
            if len(ent_idx) != 0:
                min_idx = min(ent_idx)
                max_idx = max(ent_idx)
                if min_idx != max_idx:
                    rel_ents = rel_ents[:min_idx] + rel_ents[max_idx:]  # Remove reasoning paths which contain rings
    ...
    for path in zip(res_entity_lists_new, res_path_lists_new):
        ...
        for i in range(len(path[0]) - 1):
            ...
            if output_mode == 1:
                good_episode.append(Transition(state=state_curr, action=actionID, next_state=state_next, reward=1))
            else:
                good_episode.append(Transition(state=env.idx_state(state_curr, relation=relation), action=actionID,
                                               next_state=env.idx_state(state_next, relation=relation), reward=1))
        good_episodes.append(good_episode)
    return good_episodes


def path_clean(path):  # path: relation, entity, relation, entity, relation, ..., entity
    # Remove rings from the reasoning path
    rel_ents = path.split(' -> ')
    relations = []
    entities = []
    for idx, item in enumerate(rel_ents):
