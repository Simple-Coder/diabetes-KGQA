...
class NLG():
    ...
    def generate_response(self, client, server, dialog_context):
        ...
        try:
            ...
            # 2个均为诊断：合并答案
            elif intent_enum1 == IntentEnum.Gossip and intent_enum2 == IntentEnum.Gossip:
                final_answer_text = self.gossip_robot(intent1)
            # 2个均为澄清：合并澄清
            ...
            # 1个闲聊，一个诊断：合并诊断
                ...
                final_answer_text = self.handle_medical(answer_info1)
                # 一个澄清，一个闲聊：只处理澄清
    ...
    def handle_medical(self, answer_info1):
        if not answer_info1:
