...
class KGQAProcessor():
    ...
    def search(self, query, constraint, main_entity, relation):
        # 1、子图召回
        subgraphs = self.subgraph_retriever.retrieve_subgraphs(main_entity, relation)

        # 2、答案排序和过滤
        single_sub_graph = self.subgraph_ranker.rank_and_filter_subgraphs(query, constraint, subgraphs)
