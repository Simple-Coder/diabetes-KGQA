...
class RelationMatchingModel(nn.Module):
    ...
    def forward(self, input_queries, main_entities, candidate_relations):
        # 编码输入的查询、主实体和候选关系
        input_queries_ids = self.tokenizer(input_queries, padding=True, truncation=True, return_tensors="pt")["input_ids"]
        main_entities_ids = self.tokenizer(main_entities, padding=True, truncation=True, return_tensors="pt")["input_ids"]
        candidate_relations_ids = self.tokenizer(candidate_relations, padding=True, truncation=True, return_tensors="pt")["input_ids"]
        ...
        relation_embeddings_expanded = relation_embeddings.unsqueeze(0).expand(main_entity_embeddings_expanded.shape[0], -1, -1)
